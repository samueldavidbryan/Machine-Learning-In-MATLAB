...
if exist('rawSensorData_train.mat','file') && exist('rawSensorData_test.mat','file')
    ...
    trainActivity = categorical(importdata(fullfile('UCI HAR Dataset','train','y_train.txt')),1:6,activity_labels);
    ...
    filestoload = strcat([fullfile('UCI HAR Dataset','train','Inertial Signals'),filesep],{'*gyro*','total*'});
	% Choose this if you want to load all files
    % filestoload = [fullfile('UCI HAR Dataset','train','Inertial Signals'),filesep]; 
    ...
    testActivity = categorical(importdata(fullfile('UCI HAR Dataset','test','y_test.txt')),1:6,activity_labels);
    ...
    filestoload = strcat([fullfile('UCI HAR Dataset','test','Inertial Signals'),filesep],{'*gyro*','total*'});
	% Choose this if you want to load all files
    % filestoload = [fullfile('UCI HAR Dataset','test','Inertial Signals'),filesep]; 
